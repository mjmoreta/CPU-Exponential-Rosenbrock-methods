...
for ll=1:8
    ...
    for r=1:1  
        ...
        % vecb and vecb2 contain some vectors that are multiplied by the
        ...
        vecb(:,2)=Au+(4*Uh2+2*(Phh-U.*Un2)+Un2.^2+Phh1-k*Vhn)/3;
        vecb(1,2)=vecb(1,2)+Chu1;
        vecb(N-1,2)=vecb(N-1,2)+ChuN;
        
        vecb(:,3)=vecb2(:,3);
        vecb(1,3)=vecb(1,3)+k226*Chsum1;
        vecb(N-1,3)=vecb(N-1,3)+k226*ChsumN;
